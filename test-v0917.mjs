...
assert.match(version,/APP_VERSION="0\.9\.17"/);
for(const icon of ["!!","!","★","👍","✓","📖","?!","?","❌","??"]) assert.match(app,new RegExp(`icon:"${icon.replace(/[.*+?^${}()|[\\]\\]/g,'\\$&')}"`));
assert.match(index,/id="globalTree"/); assert.match(index,/id="analysisMoves"/); assert.match(index,/id="analysisEval"/);
assert.match(app,/globalTreeSideFilter/); assert.match(app,/analysisScopeTab/); assert.match(app,/playedByUser/); assert.match(app,/line.setAttribute\("stroke","currentColor"\)/);
...
assert.match(sw,/v0\.9\.17/);
console.log('V0.9.17 ANALYSIS/ARROWS/ANNOTATION TESTS OK');
